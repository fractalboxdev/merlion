...
Out of scope: a host page that sets hostile `--merlion-*` values on its own diagrams (the host already controls its page), and denial of service by a caller that raises the limits.
...
2. **No source text in CSS.** Style statements are parsed into typed values and re-serialised; an inline SVG's `<style>` applies to the whole document, so passing source CSS through would let a diagram restyle the host page or make network requests through `url()`.
3. **Scoped selectors.** Every emitted rule is prefixed with the root's id.
...
| `render` | No panic; output is well-formed XML; output contains none of the forbidden elements, attributes or selectors; every `url(#…)` names an id defined in the same SVG |
