...
3. **Scoped selectors.** Every emitted rule starts with `#{id} `, `:where(#{id} ` or `#{id}:not(:is([data-theme="light"] *)) `. Compiled page CSS uses only the selector shapes in [svg-output.md](svg-output.md#stylesheet): it has no attribute selector other than `[data-theme="<ident>"]`, so it cannot probe host attributes.
