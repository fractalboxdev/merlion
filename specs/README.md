...
| Safety | The SVG can be inlined without a sanitiser even when the source is untrusted: no scripts, no source CSS, no selectors that reach the host page |
| Theming | Light, dark and custom themes switch by CSS alone; there is no re-render |
...
| [svg-output.md](svg-output.md) | The SVG contract: CSS variables, accessibility, ids, data attributes, escaping |
