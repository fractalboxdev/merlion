# ADR-0010: Hover interaction in two layers: bounded CSS in the SVG, full behaviour in the viewer
...
- **The viewer budget is tight.** The base element is 3.36 KB gzip of its 6 KB; the interaction behaviour below is estimated at 2.7 KB.
...
2. **Viewer layer, in `@fractalboxdev/merlion-view/interact`.** A separate module with its own ≤ 3 KB gzip budget, attached through an extension hook in the base element. It computes adjacency from `data-merlion-from`/`-to`, toggles classes on the lit set, styles them through one adopted sheet whose opacity rules override the CSS layer, and adds path mode, pinning, a popover built with `textContent`, keyboard traversal in declaration order with a polite live region, touch, and a 6 px edge hit area from a sampled-point grid.
