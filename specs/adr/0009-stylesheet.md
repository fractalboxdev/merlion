...
- `:where(#{id} .merlion-node, #{id} .merlion-edge, #{id} .merlion-cluster, #{id} marker){--merlion-tone:initial;--merlion-dash:initial}` stops ancestors, `:root` and clusters from tinting members and arrow markers. A role rule (0,1,0) beats the zero-specificity reset on the group itself, and its shape and text inherit from the group. These two per-element tokens are the only custom properties the embedded style declares; theme tokens are never declared.
...
- Automatic tones ([svg-output.md](../svg-output.md#automatic-tones)) reuse the built-in role classes with a `merlion-auto` marker, so the stylesheet compiler, palettes and themes restyle them with no new selector or token beyond `--merlion-store`; every diagram with a decision, store, terminal or top-level cluster changes colour and bytes once.
