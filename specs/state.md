...
- Notes are never dimmed and are not targets, matching the cluster rule and the sequence one ([sequence.md](sequence.md#interaction)). A note belongs to the state it points at: it lights with that state and hides with it, so a hidden state leaves no orphaned note behind. A note on a composite state travels with the member it is drawn beside — the first one declared, whose rect its connector reaches ([Note placement](#note-placement)) — so collapsing the composite takes the note with its members, and pinning the composite lights the note along with everything it holds.
- A cluster hidden inside a collapsed one is out of view: the keyboard walk skips it, and a pin on it clears when it goes. A collapsed composite is drawn dashed and stays both a target and a pin.
