...
| M2 | Stable layout ([ADR-0006](adr/0006-stable-layout.md)); `@fractalbox/merlion-wasm`; `@fractalbox/merlion-rehype`; `@fractalbox/merlion-astro` | Mean displacement across `edits` below mermaid-ELK's; WASM size recorded; fuel default calibrated ([ADR-0008](adr/0008-deterministic-work-budget.md)) | 3–4 weeks |
...
| M3b | Hover interaction ([ADR-0010](adr/0010-hover-interaction.md)): node and edge ids and the CSS layer in the core, `@fractalbox/merlion-view/interact` ([interaction.md](interaction.md)) | Every `compat` flowchart passes the hover, keyboard and popover acceptance tests; `interact` ≤ 3 KB gzip and the base viewer still ≤ 6 KB; no new axe violations; the `render` fuzz target checks the `#{id}:has(` rules | 2–3 weeks |
