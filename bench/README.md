...
- **Extraction.** Merlion: `g.merlion-node[data-merlion-id]`, `g.merlion-edge[data-merlion-from][data-merlion-to]`, `g.merlion-cluster`. Mermaid: `g.node` (id from `…flowchart-<id>-<n>`), edge paths (`.flowchart-link`, endpoints from `data-id="L_<from>_<to>_<n>"` resolved against the node ids, else the nearest node box within 30 px), `g.edgeLabel`. Node box: union of the node's shapes, transforms applied. Label: `<text>` content with line breaks as spaces, `fa:fa-*` icon tokens removed, whitespace collapsed. Curves are sampled at 12 segments.
