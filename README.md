...
Two models are recorded. `claude-sonnet-5` spends **56× the output tokens** writing SVG instead of Mermaid (215 against 12,096 on average; 67× on the largest diagrams) and draws a graph that matches the request less often (node F1 0.918 against 1.000), while keeping every edge right and all but one of 257 labels inside its shape. `google/gemma-4-31b`, run locally, states the graph almost as well as the frontier model in Mermaid (edge F1 0.966 against 1.000) and far worse in SVG (0.870), where it leaves 24 of 253 labels outside their shapes against Sonnet's 1 of 257. The penalty for being the weaker model is small in Mermaid and large in SVG, which is the case for the renderer, per [bench/results/2026-09-24-authoring.md](bench/results/2026-09-24-authoring.md).
