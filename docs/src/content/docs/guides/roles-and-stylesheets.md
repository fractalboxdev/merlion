...
Seventeen roles are styled with no stylesheet. `merlion-themes.css` defines their tones in every theme, and each reads a theme token, so setting `--merlion-danger` on a theme retunes every `danger` node and `failure` edge.
...
| `series-1` … `series-8` | Clusters | `--merlion-series-1` … `-8` | — |
...
flowchart LR
  ...
  check -->|yes| cache[(Hint cache)]
  check -->|no| work
  subgraph work[Render]
