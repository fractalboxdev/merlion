...
impl B {
    ...
    pub fn edge_full(&mut self, from: usize, to: usize, label: Option<&str>, min_len: u32, stroke: Stroke) -> usize {
...
pub fn random(seed: u64, n: usize, m: usize, clusters: usize) -> Flowchart {
    ...
    for i in 0..n {
        let label = "x".repeat(1 + r.next() % 12);
        b.shape(&format!("n{}", i), &label, shapes[r.next() % shapes.len()]);
    }
    for _ in 0..m {
        if n == 0 {
            break;
        }
        let (u, v) = (r.next() % n, r.next() % n);
        let label = if r.next() % 4 == 0 { Some("yes") } else { None };
        let len = 1 + (r.next() % 5 == 0) as u32;
        b.edge_full(u, v, label, len, Stroke::Normal);
    }
    for c in 0..clusters {
        let parent = if c > 0 && r.next() % 2 == 0 { Some(r.next() % c) } else { None };
        b.sub(&format!("s{}", c), &format!("Cluster {}", c), parent, &[]);
    }
    if clusters > 0 {
        for i in 0..n {
            if r.next() % 3 == 0 {
                let s = r.next() % clusters;
...
pub fn run_with(c: &Flowchart, opts: &RenderOptions) -> (Result<Geometry, LayoutError>, Diagnostics) {
...
pub fn on_boundary(g: &Geometry, c: &Flowchart, i: usize, px: f64, py: f64) -> bool {
    ...
    inside(shape, n.w, n.h, dx * 0.99, dy * 0.99) && !inside(shape, n.w, n.h, dx * 1.01 + dx.signum() * 0.02, dy * 1.01 + dy.signum() * 0.02)
}
...
pub fn check(c: &Flowchart, g: &Geometry) {
    ...
    let within = |x: f64, y: f64| x >= -1e-6 && y >= -1e-6 && x <= g.width + 1e-6 && y <= g.height + 1e-6;
    for (i, n) in g.nodes.iter().enumerate() {
        let (x0, y0, x1, y1) = node_box(g, i);
        assert!(within(x0, y0) && within(x1, y1), "node {} outside the drawing", i);
    ...
    for i in 0..g.nodes.len() {
        for j in i + 1..g.nodes.len() {
            assert!(!boxes_overlap(node_box(g, i), node_box(g, j)), "nodes {} and {} overlap", i, j);
        }
    }
    for (e, edge) in c.edges.iter().zip(&g.edges) {
        ...
        assert!(edge.points.len() >= 2, "edge {:?} has no route", (e.from, e.to));
        for p in &edge.points {
            assert!(p.x.is_finite() && p.y.is_finite() && within(p.x, p.y), "edge point outside");
        }
        let (s, t) = (edge.points[0], edge.points[edge.points.len() - 1]);
        assert!(on_boundary(g, c, e.from, s.x, s.y), "edge {}->{} does not start on the boundary", e.from, e.to);
        assert!(on_boundary(g, c, e.to, t.x, t.y), "edge {}->{} does not end on the boundary", e.from, e.to);
        if let Some(l) = &edge.label {
            let lb = (l.x - l.label.width / 2.0, l.y - l.label.height / 2.0, l.x + l.label.width / 2.0, l.y + l.label.height / 2.0);
            assert!(within(lb.0, lb.1) && within(lb.2, lb.3), "label outside");
            for i in 0..g.nodes.len() {
                assert!(!boxes_overlap(lb, node_box(g, i)), "edge label overlaps node {}", i);
            }
        }
    }
    for cl in &g.clusters {
        assert!(within(cl.x, cl.y) && within(cl.x + cl.w, cl.y + cl.h), "cluster outside");
    }
}
