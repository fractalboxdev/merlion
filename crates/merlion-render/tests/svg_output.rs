...
fn all_attrs(xml: &str) -> Vec<(String, String, String)> {
    let mut out = Vec::new();
    let mut rest = xml;
...
fn assert_safe(svg: &str, id: &str) {
    ...
    for sel in css_selectors(css) {
        for part in sel.split(',') {
            assert!(
                part.trim().starts_with(&format!("#{} ", id)),
                "unscoped selector {:?}",
                part
            );
        }
...
fn css_rules_read_the_same_defaults() {
    ...
    assert!(css.contains("stroke:var(--merlion-edge, var(--merlion-line, #919497))"));
